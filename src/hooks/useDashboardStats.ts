...
export function useDashboardStats() {
  ...
  const loadStats = useCallback(async () => {
    try {
      ...
      if (latestExpense?.due_date) {
        ...
        lastExpenseDate.setMinutes(
          lastExpenseDate.getMinutes() + lastExpenseDate.getTimezoneOffset(),
        );
      ...
      const monthLabel = referenceDate.toLocaleString("pt-BR", {
        month: "long",
      });
      ...
      const totalDespesas =
        despesas?.reduce((acc, curr) => acc + Number(curr.amount), 0) || 0;
      ...
      const emAndamento =
        ocorrencias?.filter((o) => o.status === "em_andamento").length || 0;
      ...
      if (user) {
        ...
        unreadCount =
          allComunicados?.filter((c) => !readIds.has(c.id)).length || 0;
      }
  ...
  }, [profile?.condominio_id, loadStats]);
