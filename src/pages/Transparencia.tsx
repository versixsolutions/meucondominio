...
export default function Transparencia() {
  ...
  async function loadKPIs() {
    ...
    try {
      ...
      const emAndamento = assembleias?.filter(a => a.status === 'em_andamento').length || 0
      
      // Próxima assembleia agendada
      const proximas = assembleias
        ?.filter(a => a.status === 'agendada' && new Date(a.data_hora) > new Date())
        .sort((a, b) => new Date(a.data_hora).getTime() - new Date(b.data_hora).getTime())
      
      const proximaData = proximas && proximas.length > 0 
        ? new Date(proximas[0].data_hora).toLocaleDateString('pt-BR', { day: '2-digit', month: '2-digit' })
        : null
      ...
      const totalMes = despesasMes?.reduce((sum, d) => sum + (d.amount || 0), 0) || 0
      const totalAno = despesasAno?.reduce((sum, d) => sum + (d.amount || 0), 0) || 0
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          >
            <div className="flex items-start justify-between mb-4">
              ...
              <div className="text-right">
                <div className="text-2xl font-bold text-purple-600">{kpis.assembleias.total}</div>
            ...
            <p className="text-sm text-gray-600 mb-4 leading-relaxed">
              Consulte editais, atas, participe de votações e acompanhe decisões da assembleia.
            </p>
            ...
            <div className="space-y-2 border-t border-purple-100 pt-4">
              {kpis.assembleias.emAndamento > 0 && (
                <div className="flex items-center justify-between text-xs">
                  ...
                  <span className="font-bold text-purple-600">{kpis.assembleias.emAndamento}</span>
                </div>
              )}
              {kpis.assembleias.proxima && (
                <div className="flex items-center justify-between text-xs">
                  <span className="text-gray-600">📅 Próxima assembleia</span>
                  <span className="font-bold text-purple-600">{kpis.assembleias.proxima}</span>
                </div>
              )}
              {!kpis.assembleias.proxima && kpis.assembleias.emAndamento === 0 && (
          ...
          >
            <div className="flex items-start justify-between mb-4">
              ...
              <div className="text-right">
                <div className="text-2xl font-bold text-green-600">{formatCurrency(kpis.financeiro.totalMes)}</div>
                <div className="text-xs text-green-400 font-medium">Este mês</div>
              </div>
            </div>
            ...
            <p className="text-sm text-gray-600 mb-4 leading-relaxed">
              Acompanhe todas as despesas, receitas e movimentações financeiras do condomínio.
            </p>
            ...
            <div className="space-y-2 border-t border-green-100 pt-4">
              <div className="flex items-center justify-between text-xs">
                ...
                <span className="font-bold text-green-600">{formatCurrency(kpis.financeiro.totalAno)}</span>
              </div>
              <div className="flex items-center justify-between text-xs">
                <span className="text-gray-600">🏷️ Categorias</span>
                <span className="font-bold text-green-600">{kpis.financeiro.categorias}</span>
              </div>
            </div>
